  ...
  return (
    <div className="max-w-4xl mx-auto pt-24 pb-12 px-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-100">Notifications</h1>
          <p className="text-sm text-zinc-500">{totalElements} total</p>
        </div>
        <div className="flex gap-2">
          {notifications.some(n => !n.read) && (
            <button onClick={handleMarkAllAsRead} className="btn-secondary text-sm px-4 py-2">
              <CheckCheck className="w-4 h-4" /> Mark all read
            </button>
          )}
          {totalElements > 0 && (
            <button onClick={handleClearAll} className="btn-secondary text-sm px-4 py-2 text-red-500 hover:bg-red-50">
          ...
          {notifications.map((notification, idx) => (
            <motion.div
              ...
              className={cn(
                ...
                !notification.read ? 'border-brand-300/60 bg-brand-50/30 dark:bg-brand-950/10' : 'hover:shadow-sm'
              )}
            >
              {/* Type icon */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="font-semibold text-zinc-900 dark:text-zinc-100">{notification.title}</p>
              ...
              <div className="flex flex-col gap-1 flex-shrink-0">
                ...
                <button
                  ...
                  className="p-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/20 text-zinc-400 hover:text-red-500 transition-colors"
                  title="Delete"
                >
                  <X className="w-4 h-4" />
                </button>
