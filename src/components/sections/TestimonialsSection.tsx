...
function TestimonialCard({ testimonial }: { testimonial: TestimonialResponse }) {
  return (
    <div className="card-base flex flex-col h-full min-w-[320px] md:min-w-[380px]">
      <Quote className="w-8 h-8 text-brand-500/40 mb-4" />

      <p className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed flex-1 mb-6">
      ...
      <div className="flex gap-1 mb-4">
        {Array(5).fill(0).map((_, i) => (
          <Star key={i} className={`w-4 h-4 ${i < testimonial.rating ? 'fill-amber-400 text-amber-400' : 'text-zinc-300 dark:text-zinc-600'}`} />
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">{testimonial.designationType}{testimonial.companyName ? ` @ ${testimonial.companyName}` : ''}</p>
        </div>
      </div>
    </div>
  );
